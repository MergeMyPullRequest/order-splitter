...
spliceOperationsFromEditDistances:function spliceOperationsFromEditDistances(distances){var i=distances.length-1;var j=distances[0].length-1;var current=distances[i][j];var edits=[];while(i>0||j>0){if(i==0){edits.push(EDIT_ADD);j--;continue;}if(j==0){edits.push(EDIT_DELETE);i--;continue;}var northWest=distances[i-1][j-1];var west=distances[i-1][j];var north=distances[i][j-1];var min=void 0;if(west<north)min=west<northWest?west:northWest;else min=north<northWest?north:northWest;if(min==northWest){if(northWest==current){edits.push(EDIT_LEAVE);}else{edits.push(EDIT_UPDATE);current=northWest;}i--;j--;}else if(min==west){edits.push(EDIT_DELETE);i--;current=west;}else{edits.push(EDIT_ADD);j--;current=north;}}edits.reverse();return edits;},/**
   ...
   */var FlattenedNodesObserver=function(){_createClass(FlattenedNodesObserver,null,[{key:"getFlattenedNodes",/**
    ...
    */value:function getFlattenedNodes(node){if(isSlot(node)){return node.assignedNodes({flatten:true});}else{return Array.from(node.childNodes).map(function(node){if(isSlot(node)){return node.assignedNodes({flatten:true});}else{return[node];}}).reduce(function(a,b){return a.concat(b);},[]);}}}]);function FlattenedNodesObserver(target,callback){var _this20=this;_classCallCheck(this,FlattenedNodesObserver);/** @type {MutationObserver} */this._shadyChildrenObserver=null;/** @type {MutationObserver} */this._nativeChildrenObserver=null;this._connected=false;this._target=target;this.callback=callback;this._effectiveNodes=[];this._observer=null;this._scheduled=false;this._boundSchedule=function(){_this20._schedule();};this.connect();this._schedule();}/**
     * Activates an observer. This method is automatically called when
     * a `FlattenedNodesObserver` is created. It should only be called to
     * re-activate an observer that has been deactivated via the `disconnect` method.
     */_createClass(FlattenedNodesObserver,[{key:"connect",value:function connect(){var _this21=this;if(isSlot(this._target)){this._listenSlots([this._target]);}else{this._listenSlots(this._target.children);if(window.ShadyDOM){this._shadyChildrenObserver=ShadyDOM.observeChildren(this._target,function(mutations){_this21._processMutations(mutations);});}else{this._nativeChildrenObserver=new MutationObserver(function(mutations){_this21._processMutations(mutations);});this._nativeChildrenObserver.observe(this._target,{childList:true});}}this._connected=true;}/**
     * Deactivates the flattened nodes observer. After calling this method
     * the observer callback will not be called when changes to flattened nodes
     * occur. The `connect` method may be subsequently called to reactivate
     * the observer.
     */},{key:"disconnect",value:function disconnect(){if(isSlot(this._target)){this._unlistenSlots([this._target]);}else{this._unlistenSlots(this._target.children);if(window.ShadyDOM&&this._shadyChildrenObserver){ShadyDOM.unobserveChildren(this._shadyChildrenObserver);this._shadyChildrenObserver=null;}else if(this._nativeChildrenObserver){this._nativeChildrenObserver.disconnect();this._nativeChildrenObserver=null;}}this._connected=false;}},{key:"_schedule",value:function _schedule(){var _this22=this;if(!this._scheduled){this._scheduled=true;Polymer.Async.microTask.run(function(){return _this22.flush();});}}},{key:"_processMutations",value:function _processMutations(mutations){this._processSlotMutations(mutations);this.flush();}},{key:"_processSlotMutations",value:function _processSlotMutations(mutations){if(mutations){for(var i=0;i<mutations.length;i++){var mutation=mutations[i];if(mutation.addedNodes){this._listenSlots(mutation.addedNodes);}if(mutation.removedNodes){this._unlistenSlots(mutation.removedNodes);}}}}/**
...
var settings=Polymer.Settings||{};settings.useShadow=!window.ShadyDOM;settings.useNativeCSSProperties=Boolean(!window.ShadyCSS||window.ShadyCSS.nativeCss);settings.useNativeCustomElements=!window.customElements.polyfillWrapFlushCallback;Polymer.Settings=settings;})();(function(){'use strict';var styleInterface=window.ShadyCSS;/**
   ...
   */Polymer.LegacyElementMixin=Polymer.dedupingMixin(function(base){/**
     ...
     */var LegacyElement=function(_legacyElementBase){_inherits(LegacyElement,_legacyElementBase);function LegacyElement(){_classCallCheck(this,LegacyElement);var _this23=_possibleConstructorReturn(this,(LegacyElement.__proto__||Object.getPrototypeOf(LegacyElement)).call(this));_this23.root=_this23;_this23.created();return _this23;}/**
...
get:function get(){return'dom-if';}},{key:"template",get:function get(){return null;}},{key:"properties",get:function get(){return{/**
         ...
         */restamp:{type:Boolean,observer:'__debounceRender'}};}}]);function DomIf(){_classCallCheck(this,DomIf);var _this27=_possibleConstructorReturn(this,(DomIf.__proto__||Object.getPrototypeOf(DomIf)).call(this));_this27.__renderDebouncer=null;_this27.__invalidProps=null;_this27.__instance=null;return _this27;}_createClass(DomIf,[{key:"__debounceRender",value:function __debounceRender(){var _this28=this;// Render is async for 2 reasons:
...
var observer=new MutationObserver(function(){if(_this29.querySelector('template')){observer.disconnect();_this29.__render();}else{throw new Error('dom-if requires a <template> child');}});observer.observe(this,{childList:true});return false;}this.__ctor=Polymer.Templatize.templatize(template,this,{// dom-if templatizer instances require `mutable: true`, as
...
var parent=c$[0].parentNode;for(var i=0,n;i<c$.length&&(n=c$[i]);i++){parent.removeChild(n);}}this.__instance=null;this.__invalidProps=null;}}},{key:"_showHideChildren",value:function _showHideChildren(){var hidden=this.__hideTemplateChildren__||!this.if;if(this.__instance){this.__instance._showHideChildren(hidden);}}}]);return DomIf;}(Polymer.Element);customElements.define(DomIf.is,DomIf);Polymer.DomIf=DomIf;})();(function(){'use strict';/**
   ...
   */var ArraySelectorMixin=Polymer.dedupingMixin(function(superClass){/**
     ...
     */var ArraySelectorMixin=function(_superClass6){_inherits(ArraySelectorMixin,_superClass6);_createClass(ArraySelectorMixin,null,[{key:"properties",get:function get(){return{/**
           ...
           */toggle:{type:Boolean,value:false}};}},{key:"observers",get:function get(){return['__updateSelection(multi, items.*)'];}}]);function ArraySelectorMixin(){_classCallCheck(this,ArraySelectorMixin);var _this30=_possibleConstructorReturn(this,(ArraySelectorMixin.__proto__||Object.getPrototypeOf(ArraySelectorMixin)).call(this));_this30.__lastItems=null;_this30.__lastMulti=null;_this30.__selectedMap=null;return _this30;}_createClass(ArraySelectorMixin,[{key:"__updateSelection",value:function __updateSelection(multi,itemsInfo){var path=itemsInfo.path;if(path=='items'){// Case 1 - items array changed, so diff against previous array and
...
var part=path.slice('items.'.length);var idx=parseInt(part,10);if(part.indexOf('.')<0&&part==idx){this.__deselectChangedIdx(idx);}}}},{key:"__applySplices",value:function __applySplices(splices){var _this31=this;var selected=this.__selectedMap;// Adjust selected indices and mark removals
...
selected.set(item,-1);}});for(var j=0;j<s.addedCount;j++){var idx=s.index+j;if(selected.has(_this31.items[idx])){selected.set(_this31.items[idx],idx);}}};for(var i=0;i<splices.length;i++){_loop4(i);}// Update linked paths
this.__updateLinks();// Remove selected items that were removed from the items array
var sidx=0;selected.forEach(function(idx,item){if(idx<0){if(_this31.multi){_this31.splice('selected',sidx,1);}else{_this31.selected=_this31.selectedItem=null;}selected.delete(item);}else{sidx++;}});}},{key:"__updateLinks",value:function __updateLinks(){var _this32=this;this.__dataLinkedPaths={};if(this.multi){var sidx=0;this.__selectedMap.forEach(function(idx){if(idx>=0){_this32.linkPaths('items.'+idx,'selected.'+sidx++);}});}else{this.__selectedMap.forEach(function(idx){_this32.linkPaths('selected','items.'+idx);_this32.linkPaths('selectedItem','items.'+idx);});}}/**
...
(function(){'use strict';var attr='include';var CustomStyleInterface=window.ShadyCSS.CustomStyleInterface;/**
   ...
   */var CustomStyle=function(_HTMLElement2){_inherits(CustomStyle,_HTMLElement2);function CustomStyle(){_classCallCheck(this,CustomStyle);var _this35=_possibleConstructorReturn(this,(CustomStyle.__proto__||Object.getPrototypeOf(CustomStyle)).call(this));_this35._style=null;CustomStyleInterface.addCustomStyle(_this35);return _this35;}/**
...
Polymer.Base=Polymer.LegacyElementMixin(HTMLElement).prototype;var ExampleOrder=function(_Polymer$Element2){_inherits(ExampleOrder,_Polymer$Element2);function ExampleOrder(){_classCallCheck(this,ExampleOrder);return _possibleConstructorReturn(this,(ExampleOrder.__proto__||Object.getPrototypeOf(ExampleOrder)).apply(this,arguments));}_createClass(ExampleOrder,[{key:"_selectItem",value:function _selectItem(e){this.$.selector.selectIndex(e.model.index);}},{key:"getData",value:function getData(){var _this37=this;fetch("/data/exampleOrders.json").then(function(stream){return stream.body.getReader().read();}).then(function(_ref){var value=_ref.value,done=_ref.done;return _this37.parse(value);}).then(function(orders){_this37.orderTypes=Array.from(orders.keys());_this37.orders=Array.from(orders.values());_this37.selectedOrder=_this37.orders[0];});}},{key:"parse",value:function parse(value){var string=value.reduce(function(acc,code){return acc+=String.fromCharCode(code);},"");var json=JSON.parse(string);var ret=new Map();for(var orderType in json){var order=json[orderType];ret.set(orderType,order.map(function(e){return"\n"+e+"\n";}));}return ret;}},{key:"ready",value:function ready(){_get(ExampleOrder.prototype.__proto__||Object.getPrototypeOf(ExampleOrder.prototype),"ready",this).call(this);this.getData();}}],[{key:"is",get:function get(){return"example-order";}},{key:"properties",get:function get(){return{orders:Array,selectedOrder:Array,orderTypes:Array};}}]);return ExampleOrder;}(Polymer.Element);window.customElements.define(ExampleOrder.is,ExampleOrder);